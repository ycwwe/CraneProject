...
public static void deleteSendMessageRecordQueue(String jobId) {
   ...
   @param  List<Map>
   */
    public void sendPickUpChassisJob(List<Map> parameters) throws InterruptedException {
       /* comLink=new ComLink("127.0.0.1",8086);*/

        List<MessageForm> list =returnDischargeMessageList(parameters);
        for(MessageForm messageForm:list){
            Thread.sleep(200);
            switch (messageForm.getReciver()){
                case "A01":
                    loggerInfo.info("XJD正在将单条作业指令发给XCC_ONE,作业指令为："+messageForm.toString());
                    ...
                    break;
                case"A02":
                    send(comLinkTwo,messageForm);
                    break;
                    default:
                        loggError.error("XCC:"+messageForm.getReciver()+"不存在");
                        continue;
            }
    ...
    private  void send(ComlinkInterface comlinkInterface,MessageForm messageForm){
        if(comlinkInterface!=null){
            if( comlinkInterface.send(new Message(messageForm.toString()))){
            ...
            loggError.error("XCC未连接,XccId:"+messageForm.getReciver());
        }
    }
    /*检测超时没回复的指令*/
            ...
            for(ConcurrentHashMap.Entry<String, Long> entry:sendTimeAndJobIdMap.entrySet()) {
                ...
                if(millis-entry.getValue()>MAXINTERVAL){
                   ...
                   if(sendMessageRecordQueue.size()!=0){/*判断是否为空，不为空则执行下一步*/
                       for(MessageForm messageForm:sendMessageRecordQueue){
                           if(messageForm.getSequenceNum()==seq){/*根据seq的数值取出相应的messageForm*/
                               switch (messageForm.getReciver()){
                                   case "A01":
                                       if(comLink!=null){
                                           if( comLink.send(new Message(messageForm.toString()))){
